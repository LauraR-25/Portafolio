    ...
    <section id="contacto" class="py-16 md:py-24">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12">
          ...
          <p class="section-subtitle">Estoy abierta a nuevas oportunidades. Contáctame y hablemos de cómo puedo ayudarte.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div class="space-y-6">
            @for (item of contactItems; track item.label) {
              <div class="card p-6 flex items-start gap-4 hover:shadow-card-hover transition-shadow duration-300">
                <div class="w-12 h-12 rounded-xl bg-olive-50 border border-mustard-200/60 flex items-center justify-center flex-shrink-0">
                  <svg class="w-5 h-5 text-olive-700" fill="currentColor" [attr.viewBox]="item.viewBox">
                    <path [attr.d]="item.path"/>
                  </svg>
                </div>
                <div>
                  <h4 class="font-semibold text-night-900 mb-1">{{ item.label }}</h4>
                  <a [attr.href]="item.href" class="text-brown-500 text-sm hover:text-olive-700 hover:underline">
                    {{ item.value }}
                  </a>
                </div>
              </div>
          ...
          <div class="card p-6 md:p-8">
            <h3 class="text-lg font-bold text-night-900 mb-6">Envíame un mensaje</h3>
            <form (ngSubmit)="onSubmit()" class="space-y-4">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label for="name" class="block text-sm font-medium text-brown-700 mb-1">Nombre</label>
                  <input type="text" id="name" [(ngModel)]="formData.name" name="name" required class="input-field" placeholder="Tu nombre">
                </div>
                <div>
                  <label for="email" class="block text-sm font-medium text-brown-700 mb-1">Email</label>
                  <input type="email" id="email" [(ngModel)]="formData.email" name="email" required class="input-field" placeholder="tu@email.com">
                </div>
              </div>
              <div>
                <label for="subject" class="block text-sm font-medium text-brown-700 mb-1">Asunto</label>
              ...
              <div>
                <label for="message" class="block text-sm font-medium text-brown-700 mb-1">Mensaje</label>
              ...
              <button type="submit" [disabled]="sent()" class="btn-primary w-full">
                @if (sent()) {
